<aside class="fixed left-0 top-0 h-full w-72 bg-gray-900 shadow-lg z-50">
    <div class="flex items-center gap-5 px-6 py-5">
        <img src="/img/logo learnify.png" alt="learnify logo" class="w-[20%]">
        <h1 class="text-white text-3xl font-bold">Learnify</h1>
    </div>
    <nav class="mt-5">
        <ul class="space-y-2">
            <li>
                <a href="/dashboard" class="flex items-center gap-2.5 px-4 py-2 text-white hover:bg-gray-700">
                    <img src="/img/toga.png" alt="toga image" class="w-[20%]">
                    <span class="text-white">Dashboard</span>
                </a>
            </li>
            <li>
                <a href="/dashboard/course" class="flex items-center gap-2.5 px-4 py-2 text-white hover:bg-gray-700">
                    <img src="/img/list-course.svg" alt="course image" class="w-[20%]">
                    <span class="text-white">Your Course</span>
                </a>
            </li>
            <li>
                <a href="/dashboard/course" class="flex items-center gap-2.5 px-4 py-2 text-white hover:bg-gray-700">
                    <img src="/img/course2.png" alt="course image" class="w-[20%]">
                    <span class="text-white">Course</span>
                </a>
            </li>
        </ul>
    </nav>
    <div class="mt-auto">
        <ul class="space-y-2">
            <li>
                <a href="/logout" class="flex items-center gap-2.5 px-4 py-2 text-white hover:bg-gray-700">
                    <img src="/img/logout.png" alt="course image" class="w-[20%]">
                    <span class="text-white">Logout</span>
                </a>
            </li>
        </ul>
    </div>
</aside>
